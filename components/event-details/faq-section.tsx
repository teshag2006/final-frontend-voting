...
export function FAQSection({ items = [] }: FAQSectionProps) {
  ...
  return (
    <section id="faq" className="scroll-mt-20">
      <h2 className="text-xl font-bold text-foreground md:text-2xl">FAQ</h2>
      <div className="mt-4 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
        <Accordion type="single" collapsible className="w-full">
          {items.map((item) => (
            ...
            >
              <AccordionTrigger className="px-5 py-4 text-sm font-medium text-foreground hover:bg-secondary/50 hover:no-underline [&[data-state=open]>svg]:rotate-90">
